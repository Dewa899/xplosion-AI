// File: api/explosion/tnt-equivalence.js
// Ini adalah backend (RAHASIA)
...
export default function handler(request, response) {
	...
	try {
		const { rho, vol, dh, eta, e_tnt, dist, pa } = request.body;
		...
		if (!Number.isFinite(Ze) || Ze <= 0) {
			return response
				.status(400)
				.json({ message: "Hasil kalkulasi tidak valid (Ze tidak positif)" });
		}
		...
		const calculateAssessments = (Po) => {
			if (!Number.isFinite(Po)) {
				return {
					impactCategory: getImpactCategory(NaN), // Dapatkan kategori 'unknown'
					injuryCategory: getInjuryRiskCategory(NaN), // Dapatkan kategori 'invalid'
					structuralKeys: [],
					structuralRangeDescKey: null,
					equipmentKey: null,
					injuryEffects: getInjuryEffects(NaN), // Dapatkan 'awaiting_input' keys
				};
			}

			const impactCategory = getImpactCategory(Po); // { nameKey, color, textColor }
			const injuryCategory = getInjuryRiskCategory(Po); // { nameKey, color, textColor }
			const structuralKeys = getDamageDescriptionKeys(Po); // [key1, key2]
			...
			const injuryEffects = getInjuryEffects(Po); // { primaryKey, secondaryKey, conclusionKey }

			return {
				// Hanya kirim key/data yang relevan
				impactCategoryNameKey: impactCategory.nameKey,
				impactColor: impactCategory.color, // <-- TAMBAHKAN INI
				impactTextColor: impactCategory.textColor, // <-- TAMBAHKAN INI
				injuryCategoryNameKey: injuryCategory.nameKey,
				injuryColor: injuryCategory.color, // <-- TAMBAHKAN INI
				injuryTextColor: injuryCategory.textColor, // <-- TAMBAHKAN INI
		...
		// --- 6. Kirim Hasil Kalkulasi + Hasil Asesmen ke Frontend ---
		response.status(200).json({
			...
			Po_crowl: Po_crowl,
			Po_alonso: Po_alonso,
			Po_sadovski: Po_sadovski,
			isAlonsoExtrapolated: isAlonsoExtrapolated,

			// Hasil Asesmen (baru ditambahkan)
			assessments: {
		...
		response
			.status(500)
			.json({ message: "Terjadi error di server saat menghitung" });
	}
}
